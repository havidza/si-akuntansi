...
//include "uuid.php";
function logUser($modul, $event, $aksi, $ver_db, $ver_ui, $content){
	$v4uuid = UUID::v4();
	$query=mysql_query("INSERT INTO log VALUES('$v4uuid','',NOW(),'$_SESSION[id_user]','$modul','$event','$aksi','$ver_db','$ver_ui','$content')");
}
...
function catatKegiatan($tabel, $kegiatan){
	$query=mysql_query("INSERT INTO log VALUES('','$tabel','$kegiatan',NOW(),'$_SESSION[id_user]')");
}
...
function ambilData($que, $table){
	...
	$fields = mysql_list_fields('orchard',$table);
	if($query){
